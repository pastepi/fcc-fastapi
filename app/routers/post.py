...
def get_posts(
    ...
    mappings = db.execute(results_q).mappings().all()

    for row in mappings:
        post = row[models.Post]
        post.votes = row["votes"]

    post_list = [row[models.Post] for row in mappings]
...
def get_post(
    ...
    mapping = db.execute(post_query).mappings().first()
    ...
    mapping[models.Post].votes = mapping["votes"]
    post = mapping[models.Post]
